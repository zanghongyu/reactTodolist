...
`.env` files **should be** checked into source control (with the exclusion of `.env*.localstore`).
...
* `.env`: Default.
* `.env.localstore`: Local overrides. **This file is loaded for all environments except test.**
* `.env.development`, `.env.test`, `.env.production`: Environment-specific settings.
* `.env.development.localstore`, `.env.test.localstore`, `.env.production.localstore`: Local overrides of environment-specific settings.

Files on the left have more priority than files on the right:

* `npm start`: `.env.development.localstore`, `.env.development`, `.env.localstore`, `.env`
* `npm run build`: `.env.production.localstore`, `.env.production`, `.env.localstore`, `.env`
* `npm test`: `.env.test.localstore`, `.env.test`, `.env` (note `.env.localstore` is missing)
...
If you are still having issues or if you’re using a more exotic environment like a cloud editor, you can bypass the host check completely by adding a line to `.env.development.localstore`. **Note that this is dangerous and exposes your machine to remote code execution from malicious websites:**
